...
export default function Contacts() {
  ...
  const processContacts = (bookings: any[]): { senders: Contact[], receivers: Contact[] } => {
  ...
  const ContactCard = ({ contact }: { contact: Contact }) => (
    <Card className="bg-white border-2 border-primary/20 hover:shadow-lg transition-shadow duration-200">
      <CardContent className="p-4 space-y-4">
        {/* Header Row - Contact Type Badge, Name, Total Amount */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Badge 
              variant={contact.type === 'sender' ? 'default' : 'secondary'}
              className={
                contact.type === 'sender' 
            ...
            >
              {contact.type === 'sender' ? (
                <><Send className="h-3 w-3 mr-1" />SENDER</>
              ) : (
                <><UserCheck className="h-3 w-3 mr-1" />RECEIVER</>
              )}
            </Badge>
            <span className="text-lg font-semibold text-foreground">{contact.name}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {contact.mostUsedCity && (
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm">
                <MapPin className="h-4 w-4 text-orange-500" />
                <span className="font-medium text-foreground">{contact.mostUsedCity}</span>
              </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium text-foreground">
                {new Date(contact.lastBooking).toLocaleDateString()}
              </span>
            </div>
            <div className="text-xs text-muted-foreground ml-6">
              Last Booking
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card 
            className="hover:shadow-lg transition-shadow bg-white"
            style={{ border: '2px solid hsl(var(--secondary) / 0.3)' }}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
