...
export default function Reports() {
  ...
  const processedData = React.useMemo(() => {
    ...
    const routeData = bookings.reduce((acc: any, booking: any) => {
      const from = booking.pickupCity || booking.senderCity || 'Unknown';
      const to = booking.deliveryCity || booking.receiverCity || 'Unknown';
      const route = `${from} → ${to}`;
      if (!acc[route]) {
        acc[route] = { route, from, to, count: 0, revenue: 0 };
      }
      acc[route].count += 1;
    ...
    return {
      ...
      avgOrderValue: dashboardStats.totalBookings > 0 ? Math.ceil((dashboardStats.revenue || 0) / dashboardStats.totalBookings) : 0,
  ...
  return (
    <div className="min-h-screen" style={{ 
      ...
      {/* Enhanced Header */}
      <div className="bg-white/90 backdrop-blur-sm border-b border-gray-200/50 p-4 mb-8">
        <div className="container mx-auto">
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
            <div className="flex-1">
              ...
              <p className="text-gray-600 mt-1 text-sm">Comprehensive insights into your business performance</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                <Button 
                  ...
                  className="border-2 hover:shadow-lg transition-all duration-200 h-9 px-3 text-sm flex-1 sm:flex-none"
                  style={{ 
      ...
      <div className="px-4 pb-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <div className="w-full overflow-hidden">
            <TabsList className="grid w-full grid-cols-2 lg:grid-cols-4 mb-6 tabs-list-fix">
              <TabsTrigger 
                value="overview" 
                className="flex items-center justify-center gap-2 px-2 py-1 h-9 rounded-md font-medium transition-all duration-300 text-sm"
                style={{
                  color: activeTab === 'overview' ? 'white' : themeColors.primary,
                  backgroundColor: activeTab === 'overview' ? themeColors.primary : 'transparent'
                }}
              >
              ...
              <TabsTrigger 
                ...
                className="flex items-center justify-center gap-2 px-2 py-1 h-9 rounded-md font-medium transition-all duration-300 text-sm"
                style={{
                  color: activeTab === 'charts' ? 'white' : themeColors.primary,
                  backgroundColor: activeTab === 'charts' ? themeColors.primary : 'transparent'
                }}
              >
              ...
              <TabsTrigger 
                ...
                className="flex items-center justify-center gap-2 px-2 py-1 h-9 rounded-md font-medium transition-all duration-300 text-sm"
                style={{
                  color: activeTab === 'routes' ? 'white' : themeColors.primary,
                  backgroundColor: activeTab === 'routes' ? themeColors.primary : 'transparent'
                }}
              >
                <MapPin className="h-4 w-4" />
                <span className="hidden sm:inline truncate">Routes</span>
              </TabsTrigger>
              <TabsTrigger 
                value="analytics" 
                className="flex items-center justify-center gap-2 px-2 py-1 h-9 rounded-md font-medium transition-all duration-300 text-sm"
                style={{
                  color: activeTab === 'analytics' ? 'white' : themeColors.primary,
                  backgroundColor: activeTab === 'analytics' ? themeColors.primary : 'transparent'
                }}
              >
          ...
          {/* Overview Tab */}
          <TabsContent value="overview" className="space-y-6 bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-6 shadow-sm">
          ...
          {/* Charts Tab */}
          <TabsContent value="charts" className="space-y-6 bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-6 shadow-sm">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                      ...
                      <span className="text-lg font-bold text-orange-900">
                        {processedData?.statusDistribution?.find(s => s.status === 'In_transit')?.count || 0}
                      </span>
                    </div>
                  </div>
                </CardContent>
          ...
          <TabsContent value="routes" className="space-y-6 bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-6 shadow-sm">
            <Card className="bg-white/80 backdrop-blur-sm">
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <div className="flex items-center">
          ...
          <TabsContent value="analytics" className="space-y-6 bg-white/80 backdrop-blur-sm rounded-lg border border-gray-200 p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white/90 backdrop-blur-sm border-2" style={{ borderColor: `${themeColors.accent}20` }}>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {processedData.topRoutes.slice(0, 6).map((route, index) => (
                      <div key={index} className="flex items-center justify-between p-4 rounded-lg border hover:shadow-md transition-all duration-200" style={{ borderColor: `${themeColors.accent}20`, backgroundColor: `${themeColors.accent}05` }}>
                        <div className="flex items-center">
                          <div className="p-2 rounded-lg mr-3" style={{ backgroundColor: `${themeColors.accent}20` }}>
                            <MapPin className="h-4 w-4" style={{ color: themeColors.accent }} />
                          </div>
                          <div>
                            <p className="font-semibold text-gray-800">{route.from} → {route.to}</p>
                            <p className="text-sm text-gray-600">{route.count} shipments</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-bold" style={{ color: themeColors.accent }}>
                            ₹{Math.ceil(route.revenue).toLocaleString()}
                          </p>
                          <p className="text-sm text-gray-500">
                            Avg: ₹{Math.ceil(route.revenue / route.count).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              <Card className="bg-white/90 backdrop-blur-sm border-2" style={{ borderColor: `${themeColors.primary}20` }}>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 rounded-lg" style={{ backgroundColor: `${themeColors.primary}10`, border: `1px solid ${themeColors.primary}30` }}>
                      <div>
                        <p className="text-sm font-medium text-gray-600">New Customers</p>
                        <p className="text-xl font-bold" style={{ color: themeColors.primary }}>
                          {Math.floor(processedData.totalBookings * 0.23)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-gray-600">This Month</p>
                        <p className="text-sm font-semibold text-green-600">+18.2%</p>
                      </div>
                    </div>

                    <div className="flex items-center justify-between p-4 rounded-lg" style={{ backgroundColor: `${themeColors.secondary}10`, border: `1px solid ${themeColors.secondary}30` }}>
                      <div>
                        <p className="text-sm font-medium text-gray-600">Repeat Customers</p>
                        <p className="text-xl font-bold" style={{ color: themeColors.secondary }}>
                          {Math.floor(processedData.totalBookings * 0.67)}
                        </p>
                      </div>
                      <div className="text-right">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg" style={{ backgroundColor: `${themeColors.accent}10`, border: `1px solid ${themeColors.accent}30` }}>
                      <div>
                        <p className="text-sm font-medium text-gray-600">Customer LTV</p>
                        <p className="text-xl font-bold" style={{ color: themeColors.accent }}>
                          ₹{Math.ceil(processedData.avgOrderValue * 4.2).toLocaleString()}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-gray-600">Avg. Lifetime</p>
                        <p className="text-sm font-semibold text-blue-600">4.2 orders</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
