...
export function AutoGPSTracker({ bookingId }: AutoGPSTrackerProps) {
  ...
  const { data: gpsInfo, isLoading } = useQuery<GPSInfo>(
    [`/api/bookings/${bookingId}/gps-info`],
    async () => {
      const response = await fetch(`/api/bookings/${bookingId}/gps-info`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch GPS info');
      }
  ...
  const { data: liveTracking } = useQuery<LiveTracking>(
    [`/api/bookings/${bookingId}/live-tracking`],
    async () => {
