...
export function NotificationCenter() {
  ...
  // Simple and effective notification sound
  const playNotificationSound = () => {
    if (!soundEnabled || !isAudioInitialized) return;
    
    try {
      // Method 1: Web Audio API (most reliable after user interaction)
  ...
  const toggleSound = () => {
    ...
    // Always play test sound when button is clicked (user interaction)
    if (newSoundState) {
      console.log('🔊 Playing test sound - sound enabled');
      // Force play test sound immediately
      playTestSound();
    } else {
  ...
  // Initialize audio when notification center is opened (user interaction)
  useEffect(() => {
    if (isOpen && !isAudioInitialized) {
      setIsAudioInitialized(true);
      console.log('🎵 Audio initialized on notification center open');
    }
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button
            ...
            onClick={() => {
              // Enable audio on user click (required by browsers)
              if (!isAudioInitialized) {
                setIsAudioInitialized(true);
                console.log('🎵 Audio enabled on notification center click');
              }
            }}
          >
