...
export default function UserAnalyticsModal({ isOpen, onClose, user }: UserAnalyticsModalProps) {
  ...
  const { data: officeAccounts = [], isLoading: officeAccountsLoading } = useQuery<OfficeAccount[]>(
    [`/api/admin/users/${user?.id}/office-accounts`],
    async () => {
      const response = await fetch(`/api/admin/users/${user?.id}/office-accounts`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch office accounts');
      }
  ...
  const { data: detailedAnalyticsRaw, isLoading: detailedAnalyticsLoading } = useQuery({
    queryKey: [`/api/admin/users/${user?.id}/detailed-analytics`],
    enabled: !!user?.id && isOpen,
    staleTime: 30000,
  });
  const detailedAnalytics = detailedAnalyticsRaw as DetailedAnalytics || {};
  ...
  const fallbackChartData = useMemo(() => {
    ...
    const last6Months: { name: string; revenue: number; bookings: number; month: number; year: number }[] = [];
    const now = new Date();
    for (let i = 5; i >= 0; i--) {
    ...
    const typeData = bookings.reduce((acc: any, booking: BookingData) => {
    ...
    // Status distribution
    const statusData = bookings.reduce((acc: any, booking: BookingData) => {
    ...
    const routeData = bookings.reduce((acc: any, booking: BookingData) => {
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    {isLoading ? (
                      ...
                      <p className="text-2xl font-bold text-secondary-900">₹{Math.ceil(Number(analytics?.totalRevenue || 0))}</p>
                    )}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    {isLoading ? (
                      ...
                      <p className="text-2xl font-bold text-secondary-900">₹{Math.ceil(Number(analytics?.monthlyCommission || 0))}</p>
                    )}
                  ...
                  <CardContent className="p-6">
                    {isLoading || detailedAnalyticsLoading ? (
                      ...
                      <ResponsiveContainer width="100%" height={250}>
                        <AreaChart data={(chartData.last6Months as any[] || []).map((item: any) => ({
                          name: item.month,
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex justify-between items-center pt-2">
                                ...
                                <div>
                                  ...
                                  <p className="font-bold text-primary-900">₹{Math.ceil(Number(agent.totalRevenue || 0))}</p>
                                </div>
                              </div>
                            </div>
                          </CardContent>
                      ...
                      {recentBookingsData.map((booking: any, index: number) => (
                        <div key={booking.id} className="flex items-center justify-between p-4 bg-muted/20 rounded-lg border border-border">
                          ...
                          <div className="text-right">
                            <p className="font-semibold text-text-primary">₹{Math.ceil(Number(booking.totalAmount))}</p>
                            <Badge 
                  ...
                  <CardContent className="p-6">
                    <div className="space-y-6">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-text-primary">₹{Math.ceil(Number(performanceMetrics?.monthlyCommission || analytics?.monthlyCommission || 0))}</span>
                      </div>
                      <div className="flex justify-between items-center">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="text-center p-4 bg-gradient-to-br from-secondary-50 to-secondary-100 rounded-lg border border-secondary-200">
                  ...
                  <p className="text-sm font-bold text-secondary-900">₹{Math.ceil(Number(selectedAgent.totalRevenue || 0))}</p>
                </div>
                ...
                <div className="text-center p-4 bg-gradient-to-br from-accent-50 to-accent-100 rounded-lg border border-accent-200">
                  ...
                  <p className="text-sm font-bold text-accent-900">₹{Math.ceil(Number(selectedAgent.monthlyCommission || 0))}</p>
                </div>
