...
import {
  ...
  Building2, PieChart as PieChartIcon, TrendingDown,
...
function AdminAnalytics() {
  ...
  const { data: comprehensiveData, isLoading: analyticsLoading } = useQuery<any>({
  ...
  const comprehensiveAnalytics = useMemo(() => {
    ...
    (data.topRoutes as Array<{ route: string; count: number; revenue: number }> | undefined)?.forEach((route) => {
    ...
    const vehicleStats: Record<string, number> = data.vehicleStatusCounts || {};
    const totalVehicles: number = Object.values(vehicleStats).reduce((sum, count) => sum + count, 0);
    const activeVehicles: number = vehicleStats.available || 0;
    const inTransitVehicles: number = vehicleStats.in_transit || 0;
    const maintenanceVehicles: number = vehicleStats.maintenance || 0;
    const utilizationRate: number = totalVehicles > 0 ? Math.ceil(((activeVehicles + inTransitVehicles) / totalVehicles) * 100) : 0;
    
    // Customer segments based on revenue
    const customerSegments = users.reduce((acc: any, user: any) => {
      const userBookings = bookings.filter((b: any) => b.userId === user.id);
      const userRevenue = Math.ceil(userBookings.reduce((sum: number, b: any) => sum + (parseFloat(b.totalAmount) || 0), 0));
    ...
    return {
      ...
      topCities,
      serviceStats: data.servicePerformance || [],
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <PieChart>
                      ...
                      >
                        {comprehensiveAnalytics.serviceStats.map((entry: any, index: number) => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white/70 backdrop-blur-sm border-primary-200 shadow-lg">
                ...
                <CardContent className="space-y-4">
                  {comprehensiveAnalytics.topRoutes.slice(0, 8).map((route: any, index: number) => (
              ...
              <Card className="bg-white/70 backdrop-blur-sm border-secondary-200 shadow-lg">
                ...
                <CardContent className="space-y-4">
                  {comprehensiveAnalytics.topCities.slice(0, 8).map((city: any, index: number) => (
          ...
          <TabsContent value="markets" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="bg-white/70 backdrop-blur-sm border-secondary-200 shadow-lg">
                ...
                <CardContent className="space-y-4">
                  {comprehensiveAnalytics.serviceStats.map((service: any, index: number) => (
          ...
          <TabsContent value="customers" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={350}>
                    <PieChart>
                      ...
                      >
                        {comprehensiveAnalytics.customerSegments.map((entry: any, index: number) => {
              ...
              <Card className="bg-white/70 backdrop-blur-sm border-secondary-200 shadow-lg">
                ...
                <CardContent className="space-y-4">
                  {comprehensiveAnalytics.customerSegments.map((segment: any, index: number) => (
