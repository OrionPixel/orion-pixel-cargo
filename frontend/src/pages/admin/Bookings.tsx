...
function AdminBookings() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ bookingId, status }: { bookingId: number; status: string }) => {
      return await apiRequest('PATCH', `/api/admin/bookings/${bookingId}/status`, { status });
    },
    onSuccess: () => {
  ...
  const stats = useMemo(() => {
    return {
      ...
      totalRevenue: Math.ceil(bookings.reduce((sum, b) => sum + Number(b.totalAmount || 0), 0))
    };
  }, [bookings]);
            ...
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-2xl font-bold text-text-primary">₹{Math.ceil(stats.totalRevenue).toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    <TableRow key={booking.id}>
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          ...
                          <div>
                            ...
                            {booking.vehicleId && (
                              <p className="text-xs text-text-secondary">Vehicle: {`V${booking.vehicleId}`}</p>
                            )}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-text-primary">
                          <p className="font-medium">{booking.pickupCity || 'N/A'}</p>
                          <p className="text-sm text-text-secondary">to {booking.deliveryCity || 'N/A'}</p>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="text-text-primary">
                      ...
                      <TableCell>
                        <Badge variant="outline">
                          {(booking as any).serviceType?.toUpperCase() || booking.bookingType?.toUpperCase() || 'FTL'}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <span className="font-semibold text-text-primary">₹{Math.ceil(Number(booking.totalAmount || 0))}</span>
                      </TableCell>
                      <TableCell>
