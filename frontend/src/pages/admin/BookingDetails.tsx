...
function AdminBookingDetails({ bookingId }: BookingDetailsProps) {
  ...
  const { data: booking, isLoading } = useQuery<Booking>({
    queryKey: [`/api/admin/bookings/${bookingId}`],
    enabled: !!user && user.role === 'admin' && !!bookingId,
  });

  const { data: vehicle } = useQuery<Vehicle>({
    queryKey: [`/api/admin/vehicles/${booking?.vehicleId}`],
    enabled: !!booking?.vehicleId,
  });

  const { data: bookingUser } = useQuery<User>({
    queryKey: [`/api/admin/users/${booking?.userId}`],
