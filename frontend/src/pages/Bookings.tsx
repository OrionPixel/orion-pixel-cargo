...
export default function Bookings() {
  ...
  const { data: bookings = [], isLoading, error, refetch } = useQuery<Booking[]>(
    ["/api/bookings"],
    {
      ...
      // gcTime is not a valid option in react-query v4, so remove it
    }
  );
      ...
      <div className="bg-card border-b border-border p-6 mb-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">Track and manage all your cargo bookings</p>
          </div>
      ...
      <div className="bg-card rounded-lg shadow-sm border-border border p-4 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder="Search by booking ID, customer, pickup/delivery location..."
              value={searchTerm}
      ...
      {filteredBookings.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredBookings.map((booking: Booking) => (
            <Card key={booking.id} className="hover:shadow-md transition-shadow bg-card" style={{ borderTop: '2px solid hsl(var(--primary) / 0.3)', borderRight: '2px solid hsl(var(--primary) / 0.3)', borderBottom: '2px solid hsl(var(--primary) / 0.3)', borderLeft: '4px solid hsl(var(--primary))' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium text-foreground">{booking.senderName || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Phone className="h-4 w-4 text-muted-foreground" />
                      <span>{booking.senderPhone || 'N/A'}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium text-foreground">{booking.pickupCity || 'N/A'}</span>
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium text-foreground">{booking.deliveryCity || 'N/A'}</span>
                    </div>
                    <div className="text-xs text-muted-foreground ml-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-green-50 p-4 rounded-lg border">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="text-green-600">{selectedBooking.pickupCity || 'N/A'}</div>
                  </div>
                  <div>
                    <span className="font-medium text-blue-800">To:</span>
                    <div className="text-green-600">{selectedBooking.deliveryCity || 'N/A'}</div>
                  </div>
                </div>
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
        <div>
          ...
          <p className="text-gray-900">{booking.pickupCity || 'N/A'}</p>
        ...
        <div>
          ...
          <p className="text-gray-900">{booking.deliveryCity || 'N/A'}</p>
          <p className="text-sm text-gray-500">
            ...
            {trackingEvents.map((event, index) => (
              <div key={index} className="flex items-start gap-4 p-4 border rounded-lg hover:bg-gray-50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-gray-600 mt-1">
                    {event.notes || (event as any).description || 'No additional details'}
                  </div>
      ...
      <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg text-sm">
        ...
        <div>
          ...
          <p className="text-gray-900">{(booking as any).serviceType || booking.bookingType || 'Standard'}</p>
        </div>
      </div>
    </div>
  );
          ...
          <div className="p-4 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg border">
            ...
            <div className="mt-3 pt-3 border-t border-blue-200">
              ...
              <div className="text-sm mt-1">
                <strong>Pickup Date:</strong> {formatTimestamp(booking.pickupDateTime?.toString() || '')}
              </div>
            </div>
          </div>
