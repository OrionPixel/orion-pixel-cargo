...
export class DatabaseStorage implements IStorage {
  ...
  async getAllVehicles(): Promise<any[]> {
    ...
    try {
      ...
      console.log('🌍 STORAGE: Node.js version:', process.version);
      console.log('🌍 STORAGE: Environment:', process.env.NODE_ENV);
      
      // Set trial end date to 14 days from now (simplified)
      const trialEndDate = new Date();
      trialEndDate.setDate(trialEndDate.getDate() + 14);
      
      console.log('📅 STORAGE: Trial end date set to:', trialEndDate);
      
      // Generate unique ID manually (with enhanced fallback for production)
      let userId;
      try {
        if (typeof crypto !== 'undefined' && crypto.randomUUID) {
          userId = crypto.randomUUID();
          console.log('🆔 STORAGE: Generated user ID with crypto:', userId);
        } else {
          throw new Error('crypto.randomUUID not available');
        }
      } catch (cryptoError) {
        // Enhanced fallback UUID generation for production environments
        console.warn('⚠️ STORAGE: Crypto.randomUUID() not available, using enhanced fallback');
        ...
        const random5 = Math.random().toString(16).substring(2, 14);
        
        userId = `${timestamp.substring(0,8)}-${random1.substring(0,4)}-4${random2.substring(0,3)}-${random3.substring(0,4)}-${random4}${random5}`.substring(0,36);
        console.log('🆔 STORAGE: Generated user ID with enhanced fallback:', userId);
      }
      
      // Prepare user data with proper database field mapping
      const userToCreate = {
        id: userId, // Add explicit ID
        ...
        trialEndDate,
        // trialStartDate will be set by database default
        role: 'transporter', // Set default role
      ...
      console.log('🗄️ STORAGE: Final user data for database:', {
        ...userToCreate,
        hashedPassword: '[HIDDEN]'
      });
      ...
      try {
        ...
        console.error('❌ STORAGE: Database connection failed:', dbError);
      ...
      // Create default theme settings for new user with performance optimization
      if (newUser && (newUser.role === 'transporter' || newUser.role === 'distributor' || newUser.role === 'warehouse' || newUser.role === 'office')) {
        try {
          // Use Promise.resolve to prevent blocking user creation
          setImmediate(async () => {
            try {
          ...
          console.error('❌ Failed to setup theme creation for user:', themeError);
        }
      }
      ...
      console.error("❌ STORAGE: Critical error creating user:", error);
